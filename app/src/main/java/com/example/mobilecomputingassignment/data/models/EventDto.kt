...
data class EventDto(
        ...
        val volume: Int = 1,
        val recentNoiseDbfs: Double? = null
) {
    fun toDomain(): Event {
        val attendeesList: List<String> =
                when (attendees) {
        ...
        return Event(
                ...
                isActive = isActive,
                volume = volume,
                recentNoiseDbfs = recentNoiseDbfs
        )
    }
    ...
    companion object {
        fun fromDomain(event: Event): EventDto {
            ...
            return EventDto(
                    ...
                    volume = event.volume,
                    recentNoiseDbfs = event.recentNoiseDbfs
            )
        }
    }
}
...
data class EventLocationDto(
    ...
    companion object {
        fun fromDomain(location: EventLocation) =
                EventLocationDto(
                        location.name,
                        location.address,
                        location.latitude,
...
data class EventAmenitiesDto(
    ...
    companion object {
        fun fromDomain(a: EventAmenities) =
                EventAmenitiesDto(
                        a.isIndoor,
                        a.isOutdoor,
                        a.isChildFriendly,
...
data class EventAccessibilityDto(
    ...
    companion object {
        fun fromDomain(a: EventAccessibility) =
                EventAccessibilityDto(
                        a.isWheelchairAccessible,
                        a.hasAccessibleToilets,
                        a.hasAccessibleParking,
                        a.hasSignLanguageSupport,
                        a.hasAudioSupport
                )
    }
}
