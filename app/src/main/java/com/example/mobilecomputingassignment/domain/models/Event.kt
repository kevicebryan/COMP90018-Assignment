 ...
 * Domain Model: Event
 *
 * This is the core business model for an event in our app. It represents a sports watch-along event
 * that users can host or join.
 *
 * Key Concepts:
 ...
 *
 * Firebase Mapping: This model gets converted to/from Firebase Firestore documents via EventDto
 */
data class Event(
        ...
        // User participation tracking - // keep legacy single-attendee string so nothing else
        // explodes
        ...
        val recentNoiseDbfs: Double? = null // Recent noise level in dBFS
) {
        /**
         * Computed Properties
         *
         * These are properties that are calculated on-the-fly rather than stored. They
         * automatically generate the event title and description based on match details.
         */

        // Auto-generates event title from match details (e.g., "GWS vs St Kilda - Sydney
        // Showground")
        val title: String
                get() =
        ...
        // Auto-generates event description from match details (e.g., "AFL Round 24 at Sydney
        // Showground")
        val description: String
                get() =
                        matchDetails?.let { match ->
                                "${match.competition} ${match.round} at ${match.venue}"
                        }
 ...
 *
 * Represents the physical location where the event takes place. Contains both human-readable
 * address and GPS coordinates for mapping.
 */
 ...
 *
 * Represents the available facilities and features at the event venue. These are boolean flags that
 ...
 *
 * Represents accessibility features available at the event venue. These help users with
 * disabilities find suitable events.
 */
 ...
 * Match Details Model
 *
 * Represents information about the AFL match associated with this event. This data comes from the
 * Squiggle API (official AFL data source).
 */
