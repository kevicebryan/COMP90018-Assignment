 ...
 *
 * Uses Australian timezone for consistent AFL event handling
 */
object EventDateUtils {

    /**
     * Check if an event is in the past (before today) Uses Australian timezone for consistent AFL
     * event handling
     * @param event The event to check
     * @return true if the event date is before today, false otherwise
     */
    fun isEventInPast(event: Event): Boolean {
        return !TimezoneUtils.isTodayOrFuture(event.date)
    }

    /**
     * Check if an event is today or in the future Uses Australian timezone for consistent AFL event
     * handling
     * @param event The event to check
    ...
    /**
     * Filter events to only include those that are today or in the future Uses Australian timezone
     * for consistent AFL event handling
